...
Subroutine reference called at start of various subs to validate arguments.

Returns C<1> automatically on MSWin32.  Returns C<undef> if arguments do not pass checks.  Returns C<1> otherwise.

We should probably retain C<$PFSCheck> but can probably drop C<$CopyLoop>.
...
B<Existing Documentation:> This function uses File::Copy's C<copy()> function to
copy a file but not a directory. Any directories are recursively created if
need be.  One difference to C<File::Copy::copy()> is that fcopy attempts to
preserve the mode (see Preserving Mode below) The optional C<$buf> in the
synopsis is the same as C<File::Copy::copy()>'s 3rd argument.  This function
returns the same as C<File::Copy::copy()> in scalar context and 1,0,0 in list
context to accomodate C<rcopy()>'s list context on regular files. (See below for
more info)

B<Comments:> We can probably eliminate C<$RMTrgFil> stuff.

Internally calls C<pathmk()>.

For the purpose of avoiding C<File::Copy::Recursive::fcopy()> in the
toolchain, do we need to handle symlinks?  If not, then we can eliminate
C<$CopyLink> and code pertaining thereto.

B<Existing Documentation:> If your system supports symlinks then symlinks will
be copied as symlinks instead of as the target file.  Perl's C<symlink()> is
used instead of File::Copy's C<copy()>.  You can customize this behavior by
setting C<$File::Copy::Recursive::CopyLink> to a true or false value.  It is
already set to true or false depending on your system's support of symlinks so
you can check it with an if statement to see how it will behave:

If symlinks are being copied you can set C<$File::Copy::Recursive::BdTrgWrn>
to true to make it carp when it copies a link whose target does not exist.
...
C<$File::Copy::Recursive::KeepMode> to false;

B<Comment:> We can probably eliminate C<$BdTrgWrn>.  We may want to
unalterably keep C<$KeepMode> true, since that's one of this module's
documented "improvements" over F<File::Copy>.
...
B<Existing documentation:> This function recursively traverses the C<$orig>
directory's structure and recursively copies it to the C<$new> directory.

    my $num_of_files_and_dirs = dircopy($orig,$new);
    my($num_of_files_and_dirs,$num_of_dirs,$depth_traversed) = dircopy($orig,$new);

C<$new> is created if necessary (multiple non existent directories is ok (i.e.
F<foo/bar/baz>). The script logically and portably creates all of them if
...
This function returns true or false: for true in scalar context it returns the
number of files and directories copied, whereas in list context it returns the
number of files and directories, number of directories only, depth level
traversed.

B<Comment:> Internally calls C<pathmk()>.

We can probably eliminate C<$CPRFComp>.

B<Existing documentation:> Removing existing target file or directory before copying.

This can be done by setting C<$File::Copy::Recursive::RMTrgFil> or
C<$File::Copy::Recursive::RMTrgDir> for file or directory behavior
respectively.
...
B<Comment:> We can probably remove this.
...
B<Existing documentation:> Creates a given path recursively. Creates
foo/bar/baz even if foo does not exist.

   File::Copy::Recursive::pathmk('foo/bar/baz') or die $!;

An optional second argument if true acts just like
C<$File::Copy::Recursive::NoFtlPth>, which means you'd never get your C<die()> if
something went wrong. Again, probably a *bad* idea.

B<Comments:>  We can probably eliminate the second argument and all code
within sub pertaining thereto.

By default, package global C<$DirPerms> is C<0777>.
