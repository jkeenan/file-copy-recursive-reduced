...
SKIP: {
    ...
    ok(! defined $rv, "rcopy() does not yet handle copying from symlink");
    #ok($rv, "rcopy() returned true value when copying from symlink");
    #ok(-f $new, "rcopy(): $new is a file");
    #ok(-l $new, "rcopy(): but $new is also another symlink");
    ...
    #    "rcopy(): Got expected warning when copying from symlink whose target does not exist");
}
